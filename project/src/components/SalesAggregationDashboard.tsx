...
export default function SalesAggregationDashboard() {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-2xl shadow p-2 sm:p-4">
        <div className="flex gap-1.5 sm:gap-2">
          <button
            ...
            className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg text-sm sm:text-base font-bold ${tab === 'sales' ? 'bg-emerald-600 text-white' : 'bg-gray-100 text-gray-700'}`}
          >
            売上集計
          </button>
          <button
            ...
            className={`px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg text-sm sm:text-base font-bold ${tab === 'analysis' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700'}`}
          >
            分析メニュー
          </button>
        </div>
      ...
      {tab === 'sales' ? (
        <div className="bg-white rounded-2xl shadow p-2 sm:p-4 space-y-2 sm:space-y-4">
          <div className="rounded-xl border border-green-200 bg-green-50 px-2 py-1.5 sm:px-3 sm:py-2 space-y-1">
            <div className="flex flex-wrap items-center gap-1.5 sm:gap-2">
              <h3 className="text-xs sm:text-base font-black text-gray-800 shrink-0 leading-tight">
                {clinicFilter === 'kawanishi' ? '川西' : clinicFilter === 'takatsuki' ? '高槻' : '全院'} 日別集計
              </h3>
              <input
                type="month"
                value={normalizeYm(month)}
                onChange={(e) => setMonth(normalizeYm(e.target.value))}
                className="px-1.5 py-1 text-xs sm:text-sm border rounded-md bg-white min-w-0"
              />
              <select
                value={clinicFilter}
                onChange={(e) => setClinicFilter(e.target.value as ClinicFilter)}
                className="px-1.5 py-1 text-xs sm:text-sm border rounded-md bg-white max-w-[7.5rem] sm:max-w-none"
              >
              ...
              <button
                type="button"
                onClick={downloadCsv}
                className="ml-auto shrink-0 px-2 py-1 rounded-md border border-emerald-400 bg-emerald-600 text-white text-[11px] sm:text-sm font-bold hover:bg-emerald-700"
              >
                CSV
              </button>
            </div>
            <div className="text-[10px] sm:text-xs text-gray-600 leading-snug">
              {loading ? (
                <span className="text-gray-500">集計中...</span>
              ) : (
                <>
                  <span>
                    月内 来院{sourceCount.visits} / 物販{sourceCount.products} / サブ{sourceCount.subs}
                  </span>
                  <span className="text-gray-400 ml-1 sm:ml-2">
                    DB 来院{rawFetched.visits} / 物販{rawFetched.products} / サブ{rawFetched.subs}
                  </span>
                </>
              )}
            </div>
